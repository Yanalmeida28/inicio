  ...
  return (
    <main className="hub-home">
      ...
      <section id="planos" className="hub-plans">
        <div className="page-container"><div className="hub-section-head"><div className="eyebrow">PLANOS PARA CRESCER</div><h2>Comece simples. Escale sem trocar de sistema.</h2><p>Escolha a estrutura que faz sentido hoje e expanda quando sua operação crescer.</p></div><div className="hub-plan-grid">{[['Essencial','R$ 197','1 filial • 3 usuários',['Vendas + Estoque','PDV e pedidos B2B','Suporte por WhatsApp']],['Profissional','R$ 397','3 filiais • 10 usuários',['Todos os módulos','Curva ABC + B2B','Gestão financeira']],['Enterprise','R$ 797','Filiais e usuários ilimitados',['API e integrações','Fiscal avançado','Suporte dedicado']]].map(([name, price, limit, features], index) => <div className={`hub-plan ${index === 1 ? 'featured' : ''}`} key={name as string}>{index === 1 && <span className="hub-plan-badge">MAIS ESCOLHIDO</span>}<strong>{name as string}</strong><small>{limit as string}</small><div className="hub-plan-price">{price as string}<em>/mês</em></div><ul>{(features as string[]).map(feature => <li key={feature}><Check size={14} /> {feature}</li>)}</ul><button onClick={openPanel}>{index === 1 ? 'Agendar demonstração' : 'Conhecer plano'} <ArrowRight size={14} /></button></div>)}</div></div>
      </section>
