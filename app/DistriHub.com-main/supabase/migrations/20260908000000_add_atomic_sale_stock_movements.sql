-- Sincroniza a RPC de venda confirmada em produção.
-- Mantém a assinatura existente e adiciona somente movimentos de estoque
-- dentro da mesma transação da baixa/estorno.
...
BEGIN
  ...
  IF v_existing_user IS NOT NULL AND v_old_status = 'cancelada' AND p_status <> 'cancelada' THEN RAISE EXCEPTION 'Venda cancelada nao pode voltar a um estado ativo'; END IF;
  IF v_existing_user IS NOT NULL AND v_old_status = 'concluida' AND p_status NOT IN ('concluida', 'cancelada') THEN RAISE EXCEPTION 'Venda concluida nao pode voltar para pre-venda'; END IF;
  ...
  IF p_payment_method = 'faturado' AND p_status = 'concluida' THEN
    ...
    IF COALESCE(v_credit_used, 0) + COALESCE(p_total, 0) > COALESCE(v_credit_limit, 0) THEN RAISE EXCEPTION 'Credito insuficiente para a venda'; END IF;
  END IF;
  ...
  IF v_should_decrement THEN
    ...
    FOR v_item IN SELECT * FROM pg_catalog.jsonb_array_elements(COALESCE(p_items, '[]'::jsonb)) LOOP
      ...
      SET stock = stock - v_quantity, updated_at = pg_catalog.now()
      ...
      SELECT v_company_id, p.id, p.name, 'saida', COALESCE((v_item->>'quantity')::integer, 0), 'Saida por venda ' || v_target_id::text, p_branch_id
  ...
  IF v_should_restore THEN
    FOR v_item IN SELECT * FROM pg_catalog.jsonb_array_elements(COALESCE(v_existing_items, '[]'::jsonb)) LOOP
      ...
      SELECT branch_id, is_service INTO v_product_branch, v_is_service
      FROM public.partner_products
      WHERE id = (v_item->>'product_id')::uuid AND user_id = v_company_id
      FOR UPDATE;
      IF v_product_branch IS NULL OR v_product_branch <> p_branch_id THEN RAISE EXCEPTION 'Produto fora da filial do estorno'; END IF;
      IF NOT COALESCE(v_is_service, false) THEN
        ...
        WHERE id = (v_item->>'product_id')::uuid
          ...
          AND branch_id = p_branch_id;
        INSERT INTO public.stock_movements (user_id, product_id, product_name, type, quantity, reason, branch_id)
        SELECT v_company_id, p.id, p.name, 'entrada', v_quantity,
...
REVOKE ALL ON FUNCTION public.execute_partner_sale_mutation(uuid, text, uuid, uuid, text, jsonb, numeric, text, text, uuid, text, text, text, text) FROM PUBLIC;
